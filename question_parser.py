...
class QuestionPaser:
    ...
    '''构建实体节点'''
    def build_entitydict(self, args):   # {'感冒': ['disease']}
        entity_dict = {}
        for arg, types in args.items():  # '感冒'，'disease'
            for type in types:
                if type not in entity_dict:
                    entity_dict[type] = [arg]   # {'disease':['感冒']}
                else:
    ...
    '''解析主函数'''
    def parser_main(self, res_classify):  # 感冒忌吃什么，{'args': {'感冒': ['disease']}, 'question_types': ['disease_not_food']}
        args = res_classify['args']  # {'感冒': ['disease']}
        entity_dict = self.build_entitydict(args)   # {'disease':['感冒']}
        question_types = res_classify['question_types'] # ['disease_not_food']
        sqls = []
        for question_type in question_types:
            sql_ = {}
            sql_['question_type'] = question_type   # {'question_type':'disease_not_food'}
            sql = []
            if question_type == 'disease_symptom':
                ...
                sql = self.sql_transfer(question_type, entity_dict.get('disease'))#sql_transfer('disease_not_food','感冒')
    ...
    '''针对不同的问题，分开进行处理'''
    def sql_transfer(self, question_type, entities):    # sql_transfer('disease_not_food','感冒')
        if not entities:
            return []
        ...
        # 查询疾病的并发症
            ...
            sql = sql1 + sql2
        # 查询疾病的忌口
        elif question_type == 'disease_not_food':    #  entities = ['感冒']。在MedicalGraph.py中 疾病的label为Disease：Node("Disease")
